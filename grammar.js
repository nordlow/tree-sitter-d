...
module.exports = grammar({
  ...
  rules: {
    ...
    // manifest (enum) compile-time constant
    constant_declaration: $ =>
      seq(
        repeat($._attribute),
        repeat($.storage_class),
    ...
    constant_declarator: $ =>
      seq(
        field('name', $._constant_identifier),
    ...
    alias_initializer: $ =>
      choice(
        seq($._alias_identifier, optional($.template_parameters), '=', repeat($.storage_class), $.type),
        seq($._alias_identifier, optional($.template_parameters), '=', repeat($.storage_class), $.function_literal),
        seq($._alias_identifier, optional($.template_parameters), '=', repeat($.storage_class), $.type, $.parameters, repeat($.member_function_attribute)),
      ),

    alias_assign: $ => seq($._alias_identifier, '=', $.type),
    ...
    // aka {primitive|builtin|fundamental|basic} type
    ...
    if_statement: $ => prec.right(seq(
      ...
      field('consequence', $.scope_statement),
      optional(seq(
        'else',
        field('alternative', $.scope_statement)
      ))
    )),
    ...
    struct_declaration: $ =>
      seq(
        ...
        'struct',
        choice(
          seq($.aggregate_body), // anonymous
          seq(field('name', $._type_identifier), choice(';', $.aggregate_body)),
          seq(
            field('name', $._type_identifier),
            $.template_parameters,
            optional($.template_constraint),
            choice(';', $.aggregate_body),
          ),
        ),
      ),
    ...
    class_declaration: $ =>
      seq(
        ...
        choice(
          ';',
          seq(optional(seq(':', $._base_class_list)), $.aggregate_body),
          seq(
            $.template_parameters,
            choice(
              ';',
              seq(optional(seq(':', $._base_class_list)), $.aggregate_body),
              seq($.template_constraint, optional(seq(':', $._base_class_list)), $.aggregate_body),
              seq(':', $._base_class_list, $.template_constraint, $.aggregate_body),
            ),
          ),
        ),
      ),
    ...
    interface_declaration: $ =>
      seq(
        repeat($._attribute),
        choice(
          seq('interface', $._type_identifier, ';'),
          seq('interface', $._type_identifier, $.aggregate_body),
          seq(
            'interface',
            $._type_identifier,
            ':',
    ...
    _enum_body: $ => seq('{', commaSep1Comma($.enum_member_declaration), '}'),
    ...
    anonymous_enum_declaration: $ =>
      seq(
        ...
        'enum',
        optional(seq(':', field('type', $._type_identifier))),
        field('body', $._anonymous_enum_body),
      ),
    ...
    // instead of `unittest_declaration` as a unittest cannot be a declaration
    unittest_definition: $ =>
      seq(repeat($._attribute), 'unittest', optional(field('name', $._string_literal)), $.compound_statement),
  },
